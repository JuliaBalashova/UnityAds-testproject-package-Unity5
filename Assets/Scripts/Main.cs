...
public class Main : MonoBehaviour {
	...
	IEnumerator PostponeVideo() {
		...
		// ios 
		Advertisement.Initialize("131623829");
		// android 
		//Advertisement.Initialize("131624296"); //always has companies
	...
	void Update() {
		...
		if(GUI.Button(new Rect(Screen.width - 380, 130, 180, 50), "Print isInitialized value\ninto log")) {
			Debug.Log("!!!!!!!!!!!!!!!!\n\n\n"+"isInitialized = " + Advertisement.isInitialized+"\n\n\n!!!!!!!!!!!!!!!!");
		...
		if(GUI.Button(new Rect(Screen.width - 380, 440, 180, 50),  Advertisement.isReady("defaultVideoAndPictureZone") ? "Show\ndefaultVideoAndPictureZone\nAds" : "Waiting when\ndefaultVideoAndPictureZone\nAds are ready")) {
